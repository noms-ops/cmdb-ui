...
PP.compareField = function(field1,field2)
{
    ...
    {
        return field1.join(',') == field2.join(',')
    }
    else
    {
...
PP.loadChart = function()
{
    ...
    var chartConfig = {
        ...
        series: [
        {
            ...
            tooltip:
            {
                ...
                {
                     this.setHtml(storeItem.get('Category') + ': ' + storeItem.get('Count'));
                }
            },
            highlight: {
              fillStyle: 'yellow'
    ...
    var modelfields = [];
    var columns = [];
    // if sub dividing  then process for that 
    if (key2)
    {
    ...
    chartConfig.series[0].yField = key2 ? keys_2 : ['Count'];
    chartConfig.series[0].label.field = key2 ? keys_2 : 'Count';
    ...
    if (key2)
    {
        chartConfig.series[0].tooltip.renderer = function(storeItem, item)
        {
            this.setHtml(storeItem.get('Category') + '- ' + item.field + ': ' + item.record.get(item.field));
        }
        PP.log('starting loop of loops');
        chartConfig.series[0].title=keys_2;
        chartConfig.series[0].yField=keys_2;
        for (var i = 0; i < keys_2.length; i++)
        {
            ...
            Ext.Object.each(PP.getVals(invStore, field, key1, [key2, keys_2[i]], count_type), function(k, v)
            {
                ...
                if (chartrec.get('Count') == '' || chartrec.get('Count') == undefined)
                {
                ...
                chartrec.dirty=false;
            });
        }
    }
    chartConfig.store = chartstore;
    // update axis titles
    chartConfig.axes[0].title = {text:count_type + ' of ' + (field == 'fqdn' ? 'Systems' : PP.getFieldLabel(field, 'system')) }; // Y axis (vertical)
    chartConfig.axes[1].title = {text: PP.getFieldLabel(key1, 'system')}; // X axis (horizonal)
    PP.log(chartConfig);
    Ext.getCmp('chartcontainer').chart=Ext.create('Ext.chart.CartesianChart',chartConfig)
    Ext.getCmp('chartcontainer').add(Ext.getCmp('chartcontainer').chart);
    Ext.getCmp('chart_grid').reconfigure(chartstore, columns);

    return;
}
PP.highlightBar = function(storeItem)
{
    var chart = Ext.getCmp('chartcontainer').chart;
    var series = chart.series[0];
    var store = Ext.getCmp('chart_grid').getStore();
    chart.setHighlightItem(series.getItemByIndex(store.indexOf(storeItem)));
}
