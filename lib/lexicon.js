...
// below items are for backward compatibility with otehr systems that may load 
...
Ext.define('PP.tagfield',{
   extend: 'Ext.form.field.Tag',
   xtype: 'tagfieldspaces',
   getSubmitData: function() {
       var me = this,
           val = me.callParent(arguments);
       if(Array.isArray(val['tags']))
       {
           return {tags: val['tags'].join(' ')};
       }
       return val;
   },
   getValue: function()
...
PP.getEnumeratorStore = function(entity,field)
{
    ...
    var storeconfig={
        ...
        proxy: {
            type: 'rest',
            url: PP.config.cmdb_api_path + 'column_lkup/' + PP.entities[entity][field]._enumeration['entity'] + '/' + PP.entities[entity][field]._enumeration['attribute'] ,
            reader: ( entity == 'system' && field == 'tags' ? { // special case for tags field
                type: 'array',
                transform: function(data) {
                    // this transform splits up space separated 'tag' so there 
                    // are not duplicates, as the values retreived from 
                    // the column_lkup api will be space delimited values
                    var newdata=[];
                    var lkup={};
                    for(var i=0; i < data.length; i++)
                    {
                        if(data[i].match(' '))
                        {
                            var multiple = data[i].split(' ');
                            for(var k=0; k< multiple.length;k++)
                            {
                                if(lkup[multiple[k]] != true)
                                {
                                    newdata.push([multiple[k]]);     
                                    lkup[multiple[k]] = true;                       
                                }
                            }
                        }
                        else
                ...
                // this transform puts the values into array of arrays
                // which is what the normal array reader wants
                type: 'array',
                transform: function(data) {
                    for(var i=0; i < data.length; i++)
                    {
...
PP.getEditorConfig = function(editor_name, opts)
{
    ...
    else
    {
        ...
        if (!PP.models.base.schema.getEntity('enumerator'))
        {
        ...
        if (!PP.models.base.schema.getEntity('arraymenu'))
        {
        ...
        if (!PP.models.base.schema.getEntity('roles'))
        {
            Ext.define('roles',
            {
                extend: 'Ext.data.Model',
        ...
        {
            // use tagfield type for multiselect
            if(PP.entities[opts.entity][editor_name]._format == 'multiselect')
            {
                PP.editors[editor_name] = { //new Ext.form.ComboBox({
                    xtype:  (editor_name == 'tags' ? 'tagfieldspaces' : 'tagfield'),
                    anyMatch: true,
                    clearFilterOnBlue: true,
                    filterPickList: true,
                    grow: true,
                    // growMin: 100,
                    // growMax: 100,
                    ...
                    triggerAction: 'all',
                    triggerOnClick: true,
                    createNewOnBlur: (editor_name == 'tags' ? true : false), // special case for tags field
                    createNewOnEnter: (editor_name == 'tags' ? true : false), // special case for tags field
                    delimiter: (editor_name == 'tags' ?  ' ' : ',') // special case for tags field
                }; //);
                PP.log('made tagfield for ' + editor_name);
            }
            else
            {
                PP.editors[editor_name] = { //new Ext.form.ComboBox({
            ...
            exception: function(proxy, response, operation)
            {
                if (operation)
                {
                    if (operation['error'] && operation['error']['status'])
                        Ext.Msg.alert("Remote Object Error: " + model.name, operation.error.status + ": " + response.responseText);
                }
                else
                {
