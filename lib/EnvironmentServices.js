...
Ext.define('PP.EnvironmentServicesProxy',
{
    ...
    buildUrl: function(request)
    {
        ...
        var lkup_env = this.getEnvironment();
        if(lkup_env == undefined) 
        {
            lkup_env = Ext.getCmp('service_grid').selectedEnvironment.get('name');
        }
        url = PP.config.cmdb_api_path + 'environments/' + lkup_env + '/services';
        request.setUrl(url);
        return PP.EnvironmentServicesProxy.superclass.buildUrl.call(this, request);
    }
});

// redefine environmentservice model proxy

PP.models.base.schema.getEntity('environmentservice').prototype.self.setProxy({type:"environmentservices"});

// var model_config = PP.make_entity_model_config('environmentservice');
// model_config.proxy = { type: 'environmentservices'};
// Ext.define('environmentservice',model_config);
// construct special environmentservices proxy to use with environments api
                ...
                Ext.Msg.show(
                {
                    ...
                    fn: function(btn)
                    {
                        if (btn == 'yes')
                        {
                            ...
                            var p = Ext.create('service_instance',
                            {
...
//             if(rec.setProxy == undefined)
//             {
//                 // not sure how, but sometimes the model has no setProxy method?
//                 rec.proxy=this.store.getProxy();
//             }
//             else
//             {
...
//                             ": " + op.error.statusText + "<br>" + op.error.response.responseText);
//                         rec.reject();
//                     }
//                 },
//                 scope: this
...
//         }, //edit::funtion
            ...
            Ext.Msg.confirm('Confirm', 'Are you sure you want to delete this record?<br>This cannot be undone', function(btn)
            {
                if (btn == 'yes')
                {
                    ...
                    rec.erase(
                    {
                        ...
                        failure: function(records, op, success)
                        {

                            Ext.Msg.alert('Error', 'There was an error deleting the record in the remote store: <br> ' + op.error.response.responseText);
                        },
                        scope: this
                    });
                }
    ...
    setEnvironment: function(env)
    {
        ...
        this.getStore().getProxy().setEnvironment(env.get('name'));  
    },
    loadEnvironment: function(env)
    {
        ...
        this.setEnvironment(this.env_store.findRecord('name', env, 0, false, true, true));
        this.store.load();
