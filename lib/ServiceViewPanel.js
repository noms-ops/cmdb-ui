...
function inheritedTip(val, meta, rec, rowIndex, colIndex, store) {
    ...
    var svc_record_name;
    var svc_attr;
    if(val.match(/service_data\(/) && typeof rec.data !='undefined')
    {
        try {
            ...
            if(val.match(/service_data\("(.*?)","(.*)"/))
            {
                ...
                tiptext+= 'Dereferenced value: ' + svc_record.data[svc_attr].value + '<br>';
            }
        }
        catch(err)
        {
            PP.log('erorr in renderer: ' + err);
        }
    }
    if(typeof rec.data.inherited !="undefined")
    {
...
Ext.define('PP.ServiceViewPanel',
{
    ...
    initComponent: function()
    {
        ...
        this.grid = new Ext.grid.Panel(
        {
            ...
            tbar: (this.editing ? [
            {
                ...
                handler: function()
                {
                    var p = Ext.create('service_record_data',{name:'',value:''});
                    var g = this.grid;
                    var svp = this;
                    if (svp.saveButton)
                    {
        ...
        if (PP.allowEditing && this.getForm().getRecord())
        {
            ...
            Ext.Ajax.request(
            {
                ...
                success: function(response, opts)
                {
                    ...
                    var updated_rec = Ext.create('service_record_data',Ext.decode(response.responseText));
                    delete updated_rec.data.id;
    ...
    loadData: function()
    {
        ...
        var keys = Object.keys(this.selectedService.data);
        var ds_recs = [];
        for (var i = 0; i < keys.length; i++)
        {
            ...
            {
                name: keys[i],
                environment_name: this.selectedService.data[keys[i]].environment_name,
                value: this.selectedService.data[keys[i]].value,
                inherited: this.selectedService.data[keys[i]].inherited
            });
        }
