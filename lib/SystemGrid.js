...
Ext.define('PP.SystemProxy',{
    ...
    buildUrl: function(request)
    {
        ...
        if (request.getParams().query)
        {
            ...
            url += request.getParams().query
            request.url = url;
            delete request.getParams().query;
        }
        return url;
    },
    listeners:
...
Ext.define('PP.SystemGrid',
{
    ...
    plugins: [
        Ext.create('Ext.grid.plugin.RowEditing',
        {
            ...
            listeners:
            {
                ...
                {
                    if(ed.editor.getFloatingButtons().rendered)
                    {
                        Ext.defer(ed.editor.getFloatingButtons().setButtonPosition,10,ed.editor.getFloatingButtons(),['bottom']);                            
                    }
                    if(!this.editor.down('button').hasListener('mouseover') )
                    {
                        ...
                        updatebutton.on('mouseover',function(){
                            ...
                            for(var field in changes)
                            {
                                ...
                                {
                                    if(!PP.compareField(changes[field],r.data[field]))
                                    {
    ...
    initComponent: function()
    {
        ...
        {
            var default_grid_fields = PP.config.default_grid_fields ||  ['fqdn', 'ipaddress', 'status', 'roles', 'environment_name', 'svc_id', 'tags', 'notes'];
            Ext.each(default_grid_fields, function(ff)
            {
    ...
    columnConfig: function(field)
    {
        ...
        return {
            ...
            renderer: (field == 'tags' ? PP.tagRenderer : undefined ), // special formatting for tags field
            field: editorConfig
        };
    },
